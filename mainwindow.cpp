...
void MainWindow::on_btnSaveConfig_clicked()
{
    ...
    transmit.sendEncoderConfig(resolutionValue, direction, speed);
}

void MainWindow::on_btnStart_clicked()
{
    qDebug() << "start butonuna basıldı";
    ui->btnStart->setDisabled(true);
    ui->btnStop->setDisabled(false);
    transmit.sendEncoderStatus(encoderStatusStart);
    ui->lcdNumber->display(100);
}

void MainWindow::on_btnStop_clicked()
{
    qDebug() << "stop butonuna basıldı";
    ui->btnStart->setDisabled(false);
    ui->btnStop->setDisabled(true);
    transmit.sendEncoderStatus(encoderStatusStop);
}
